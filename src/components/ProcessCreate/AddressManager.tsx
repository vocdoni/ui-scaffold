...
import { errorToString } from '@vocdoni/react-providers'
import { Trans, useTranslation } from 'react-i18next'
import { RiFileExcel2Line } from 'react-icons/ri'
import { addressTextOverflow, fieldMapErrorMessage, isInvalidFieldMap } from '~constants'
import { useDropzone } from 'react-dropzone'
import { EmailAddressesSpreadsheetManager } from './Census/Spreadsheet/EmailAddressesSpreadsheetManager'
import { Web3CensusSpreadsheetManager } from './Census/Spreadsheet/Web3CensusSpreadsheetManager'
...
type TypeConfig = {
  pattern: RegExp
  spreadsheetManager: new (file: any, flag: boolean) => any
...
const typeConfigs: Record<string, TypeConfig> = {
  email: {
    pattern: /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/i,
    spreadsheetManager: EmailAddressesSpreadsheetManager,
    acceptedTypes: EmailAddressesSpreadsheetManager.AcceptedTypes,
  },
  web3: {
    pattern: /^(0x)?[0-9a-f]{40}$/i,
    spreadsheetManager: Web3CensusSpreadsheetManager,
    acceptedTypes: Web3CensusSpreadsheetManager.AcceptedTypes,
  },
}
...
export const AddressManager = (props: AddressManagerProps) => {
  ...
  }, [props.initialUsers, initialized])
  ...
  const handleAddAddress = async () => {
    ...
    let pattern: RegExp = typeConfigs[props.type].pattern
    if (!pattern.test(newAddress)) {
      return setError('newAddress', { type: 'custom', message: t('form.error.email_address_pattern') })
    }
    ...
    if (!errors.newAddress) {
      ...
      if (typeof props.onUpdateSelection === 'function') {
        props.onUpdateSelection(naddresses)
      }
    }
  }
  ...
  const onDrop = async ([file]: File[]) => {
    try {
      ...
      const spreadsheet: CensusSpreadsheetManager = new typeConfigs[props.type].spreadsheetManager(file, false)
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    multiple: false,
    accept: typeConfigs[props.type].acceptedTypes.reduce((prev, curr) => ({ ...prev, [curr]: [] }), {}),
  })
