...
Vocdoni UI Scaffold is a multi-tenant SaaS platform for creating and managing blockchain-based voting processes. The application enables organizations to create secure, transparent elections.
...
- When adding new components or new code with copies, never hardcode the text. Generate a new string in a place that make sense inside of the .json files for each language. Making sure that the translations also make sense.
