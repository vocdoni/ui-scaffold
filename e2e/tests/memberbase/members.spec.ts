...
test.describe('Memberbase', () => {
  ...
  test('delete selected members while keeping one, then cleanup the last one', async ({ page }) => {
    ...
    await page.locator('tr', { hasText: memberA.email }).getByRole('checkbox').first().check({ force: true })
    await page.locator('tr', { hasText: memberB.email }).getByRole('checkbox').first().check({ force: true })
